...
- (void)getSectionAndRow {
  ...
  NSArray *fileList =
      [fm contentsOfDirectoryAtPath:[self.record getDoc] error:nil];
  ...
  for (NSString *bigdoc in bigDoc) {
    ...
    NSArray *fileList1 =
        [fm contentsOfDirectoryAtPath:[NSString
                                          stringWithFormat:@"%@/%@",
                                                           [self.record getDoc],
                                                           bigdoc]
                                error:nil];
    ...
    for (NSString *file in fileList1) {
      NSString *path1 =
          [[NSString stringWithFormat:@"%@/%@", [self.record getDoc], bigdoc]
    ...
    for (NSString *smalldoc in smallDoc) {
      NSString *path2 =
          [NSString stringWithFormat:@"%@/%@/%@/setting.txt",
                                     [self.record getDoc], bigdoc, smalldoc];
...
- (void)myHandleTableviewCellLongPressed:
  ...
  if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
    NSIndexPath *indexPath =
        [self.collectionView indexPathForItemAtPoint:pointTouch];
    NSArray *keys = [self.dataSource allKeys];
    NSMutableArray *values =
        [self.dataSource objectForKey:[keys objectAtIndex:indexPath.section]];
    //删除沙盒文件
    NSFileManager *fm = [NSFileManager defaultManager];
    NSString *path =
        [NSString stringWithFormat:@"%@/%@/%@", [self.record getDoc],
    ...
    NSArray *newbigFileList =
        [fm contentsOfDirectoryAtPath:bigDirPath error:nil];
...
- (UICollectionViewFlowLayout *)flowLayout {
  ...
  flowLayout.headerReferenceSize =
      CGSizeMake(SCREEN_WIDTH, 20.0f); //设置head大小
  return flowLayout;
}
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
  ...
  UILabel *headerLabel = [[UILabel alloc]
      initWithFrame:CGRectMake(SCREEN_WIDTH / 2 - 50, 5, 100, 20)];
  headerLabel.backgroundColor = [UIColor blackColor];
  headerLabel.text = key;
  headerLabel.textAlignment = NSTextAlignmentCenter;
  headerLabel.textColor = [UIColor colorWithRed:143.0 / 255.0
                                          green:142.0 / 255.0
                                           blue:148.0 / 255.0
                                          alpha:1.0];
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  NSString *path =
      [NSString stringWithFormat:@"%@/%@/%@/setting.txt", [self.record getDoc],
                                 bigDocName, smallDocName];
  NSArray *setting = [[NSArray alloc] initWithContentsOfFile:path];
  DiaryViewController *DVC =
      [self.storyboard instantiateViewControllerWithIdentifier:@"sixth_id"];
  DVC.setting = setting;
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [self.navigationController.navigationBar
      setBackgroundImage:[UIImage imageNamed:@"Nav Bar"]
  ...
  [doneBtn setImage:[UIImage imageNamed:@"Camera"]
